...
import TypeaheadComponent from 'ember-osf/components/search-facet-typeahead/component';
...
export default TypeaheadComponent.extend({
    ...
    actions: {
        changeFilter(selected) {
            let currentSelected = Ember.$.extend(true, [], this.get('state'));
            if (typeof selected === 'object') {
                return this._super(selected);
            }
            let checked = event.target.checked;
            if (checked === undefined) checked = true;
            const hasItem = currentSelected.includes(selected);

            if (!hasItem && selected !== undefined && checked) {
                currentSelected.pushObject(selected); // Item is checked, and needs to be part of query
            } else if (hasItem && !checked) {
                currentSelected.removeObject(selected); // Item is not checked and needs to be removed from query
            }

            return this._super(currentSelected);
        }
    }
});
